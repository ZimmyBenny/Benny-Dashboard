...
describe('Manufacturer Samples API', () => {
  ...
  it('legt Sample an, patcht Felder, liest verschachtelt in der Hersteller-Liste', async () => {
    ...
    await request(app).patch(`/api/amazon/products/${pid}/manufacturers/${mId}/samples/${sId}`)
      .send({ bezeichnung: 'Charge A', rating: 4, status: 'erhalten', is_favorite: 1, kosten: '40,23', currency: 'USD', maengel: 'Stangendicke', sendungsnummer: 'CN123456789DE', link_url: 'https://t.17track.net/abc' }).expect(200);
    ...
    expect(m.samples[0].link_url).toBe('https://t.17track.net/abc');
  });
