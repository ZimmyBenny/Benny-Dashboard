-- Bereich 'Vermietung' zum area-CHECK hinzufügen
-- SQLite erlaubt kein ALTER COLUMN → Tabelle neu erstellen (Muster wie Migration 021)
-- Muster CREATE _new → INSERT → DROP → RENAME _new ist FK-sicher:
-- Kind-Tabellen (attachments, activity_log, receipts, doc_files) verweisen auf den
-- Originalnamen, der nie umbenannt wird.
...
CREATE TABLE contracts_and_deadlines_new (
  ...
  area TEXT NOT NULL DEFAULT 'Sonstiges' CHECK(area IN ('Privat','DJ','Amazon','Cashback','Finanzen','Banken','Vermietung','Sonstiges')),
  ...
  amount_type TEXT DEFAULT 'brutto',
  vat_rate REAL DEFAULT 19,
  cancellation_notice_weeks INTEGER NOT NULL DEFAULT 4,
