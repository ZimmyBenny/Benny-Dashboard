...
/** Offene DJ-Aufgaben — identischer Baustein wie AmazonOpenTasks, gefiltert auf area 'DJ'. */
export function DjOpenTasks() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [slideOverOpen, setSlideOverOpen] = useState(false);

  const { data, isLoading, isError } = useQuery({
    queryKey: ['tasks', 'dj-open'],
    queryFn: () => fetchTasks({ area: 'DJ' }),
  });
